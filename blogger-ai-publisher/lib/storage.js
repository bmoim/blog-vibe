...
const GOOGLE_CONFIG_FILE = path.join(dataDirectory, "google-oauth-config.json");
...
function encryptionKey() { const secret = process.env.TOKEN_ENCRYPTION_KEY; return secret ? crypto.createHash("sha256").update(secret).digest() : null; }
...
function decryptRecord(record) {
  ...
  const key = encryptionKey(); if (!key) throw new Error("TOKEN_ENCRYPTION_KEY가 없어 저장된 보안 정보를 복호화할 수 없습니다.");
...
export async function readGoogleToken() { return decryptRecord(await readJson(TOKEN_FILE, null)); }
export async function saveGoogleToken(token) { await writeJsonAtomic(TOKEN_FILE, encryptRecord(token)); }
export async function removeGoogleToken() { try { await fs.unlink(TOKEN_FILE); } catch (error) { if (error.code !== "ENOENT") throw error; } }
export async function readGoogleOAuthConfig() { return decryptRecord(await readJson(GOOGLE_CONFIG_FILE, null)); }
export async function saveGoogleOAuthConfig(config) { await writeJsonAtomic(GOOGLE_CONFIG_FILE, encryptRecord(config)); }
export async function removeGoogleOAuthConfig() { try { await fs.unlink(GOOGLE_CONFIG_FILE); } catch (error) { if (error.code !== "ENOENT") throw error; } }
